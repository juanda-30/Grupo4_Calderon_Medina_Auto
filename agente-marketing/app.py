...
- Pain: 3 obstáculos de marketing que enfrenta HOY
...
Formato: subtítulos en negrita + viñetas. Máximo 500 palabras. Todo en español."""),
        HumanMessage(content=f"""Analiza este cliente potencial:

━━━ DATOS DEL CLIENTE ━━━
Empresa: {state['client_company']}
Sector: {state['client_industry'] or 'No especificado'}
Contacto: {state['client_name']}
Necesidad: {state['client_need'] or 'No especificada'}
Público objetivo: {state['target_audience'] or 'No especificado'}
Servicio de interés: {state['service_type']}

Genera el análisis completo. Sé específico para ESTE cliente."""),
    ]
    response = llm.invoke(messages)
    return {"market_analysis": response.content}

# ══════════════════════════════════════════════
# AGENTE 2 — Estratega de Marca
# ══════════════════════════════════════════════
...
- Proof 3: Garantía o mecanismo de rendición de cuentas

**KPIs de Éxito (3-5, de negocio):**
- Priorizar: conversión, costo por lead, ROI, tasa de retención

**Manejo de Objeciones (top 3):**
- Objeción → Contraargumento persuasivo

Formato: subtítulos en negrita + viñetas. Máximo 520 palabras. Todo en español."""),
        HumanMessage(content=f"""Con base en este análisis:
...
Define la estrategia aplicando el Messaging Hierarchy Canvas.
El Core Message debe ser específico para {state['client_company']}."""),
    ]
    response = llm.invoke(messages)
    return {"brand_strategy": response.content}

# ══════════════════════════════════════════════
# AGENTE 3 — Redactor Comercial
# ══════════════════════════════════════════════
...
Tu misión es integrar el análisis y la estrategia en una narrativa persuasiva de 11 secciones.

Reglas de redacción:
- El nombre del cliente aparece al menos 3 veces
- Los beneficios son tangibles y cuantificables
- Tono consultivo: enfocado en el cliente, no en la agencia
- Mínimo 850 palabras en el cuerpo
- Usa Markdown limpio: ## para secciones, **bold** para énfasis, - para listas
- Todo en español."""),
        HumanMessage(content=f"""Redacta una propuesta comercial completa usando:
...
6. Metodología y Plan de Acción
...
**P — Personalización:** ¿Parece escrita para ESTE cliente específico?
**A — Accionabilidad:** ¿El cliente sabe exactamente qué hacer después de leerla?
**C — Claridad:** ¿Cada sección puede leerse en 60 segundos?
**T — Tono:** ¿Es consultivo o vendedor? Ajusta hacia consultivo.

REGLA CRÍTICA: Devuelve la propuesta COMPLETA y mejorada. NUNCA recortes contenido."""),
        HumanMessage(content=f"""Revisa y mejora esta propuesta aplicando la rúbrica PACT:

{state['proposal_draft']}

━━━ CRITERIOS ━━━
1. ¿{state['client_company']} y su sector están integrados naturalmente?
2. ¿Los beneficios son concretos y medibles?
3. ¿El tono es consultivo?
4. ¿"Próximos Pasos" tiene verbo imperativo y plazo?
5. ¿El Markdown es consistente?
...
st.markdown(
    ...
    "El equipo de agentes usará esta información para generar una propuesta personalizada.</div>",
    unsafe_allow_html=True,
)
...
st.markdown("#### 🏢 Tu Agencia (quien envía la propuesta)")
...
if generate_btn:
    ...
    if missing:
        ...
        if not api_key:
            ...
            try:
                with st.status("🤖 Equipo de agentes trabajando...", expanded=True) as main_status:
                    ...
                    main_status.update(label="✅ ¡Propuesta completada!", state="complete")
...
if "proposal" in st.session_state:
    st.markdown("---")
    st.markdown(
        '<div class="success-banner">✅ ¡Propuesta generada! Revísala, edítala o descárgala.</div>',
        unsafe_allow_html=True,
    )
